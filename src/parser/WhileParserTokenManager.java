/* Generated By:JavaCC: Do not edit this line. WhileParserTokenManager.java */
package parser;

/** Token Manager. */
public class WhileParserTokenManager implements WhileParserConstants {

	/** Debug output. */
	public java.io.PrintStream debugStream = System.out;

	/** Set debug output. */
	public void setDebugStream(java.io.PrintStream ds) {
		debugStream = ds;
	}

	private final int jjStopStringLiteralDfa_0(int pos, long active0) {
		switch (pos) {
		case 0:
			if ((active0 & 0x8000000L) != 0L)
				return 2;
			if ((active0 & 0x2000000L) != 0L)
				return 6;
			if ((active0 & 0xfc0007000L) != 0L) {
				jjmatchedKind = 36;
				return 4;
			}
			return -1;
		case 1:
			if ((active0 & 0x480000000L) != 0L)
				return 4;
			if ((active0 & 0xb40007000L) != 0L) {
				jjmatchedKind = 36;
				jjmatchedPos = 1;
				return 4;
			}
			return -1;
		case 2:
			if ((active0 & 0x800000000L) != 0L)
				return 4;
			if ((active0 & 0x340007000L) != 0L) {
				jjmatchedKind = 36;
				jjmatchedPos = 2;
				return 4;
			}
			return -1;
		case 3:
			if ((active0 & 0x40003000L) != 0L)
				return 4;
			if ((active0 & 0x300004000L) != 0L) {
				jjmatchedKind = 36;
				jjmatchedPos = 3;
				return 4;
			}
			return -1;
		case 4:
			if ((active0 & 0x100004000L) != 0L)
				return 4;
			if ((active0 & 0x200000000L) != 0L) {
				jjmatchedKind = 36;
				jjmatchedPos = 4;
				return 4;
			}
			return -1;
		default:
			return -1;
		}
	}

	private final int jjStartNfa_0(int pos, long active0) {
		return this.jjMoveNfa_0(this.jjStopStringLiteralDfa_0(pos, active0), pos + 1);
	}

	private int jjStopAtPos(int pos, int kind) {
		jjmatchedKind = kind;
		jjmatchedPos = pos;
		return pos + 1;
	}

	private int jjMoveStringLiteralDfa0_0() {
		switch (curChar) {
		case 33:
			return this.jjStopAtPos(0, 15);
		case 38:
			return this.jjMoveStringLiteralDfa1_0(0x100000L);
		case 40:
			return this.jjStopAtPos(0, 10);
		case 41:
			return this.jjStopAtPos(0, 11);
		case 42:
			return this.jjStopAtPos(0, 28);
		case 43:
			return this.jjStopAtPos(0, 26);
		case 44:
			return this.jjStopAtPos(0, 29);
		case 45:
			return this.jjStartNfaWithStates_0(0, 27, 2);
		case 47:
			return this.jjStartNfaWithStates_0(0, 25, 6);
		case 59:
			return this.jjStopAtPos(0, 18);
		case 60:
			return this.jjStopAtPos(0, 22);
		case 61:
			jjmatchedKind = 19;
			return this.jjMoveStringLiteralDfa1_0(0x1000000L);
		case 62:
			return this.jjStopAtPos(0, 23);
		case 101:
			return this.jjMoveStringLiteralDfa1_0(0x40000000L);
		case 102:
			return this.jjMoveStringLiteralDfa1_0(0x4000L);
		case 105:
			return this.jjMoveStringLiteralDfa1_0(0x480000000L);
		case 111:
			return this.jjMoveStringLiteralDfa1_0(0x800000000L);
		case 112:
			return this.jjMoveStringLiteralDfa1_0(0x1000L);
		case 114:
			return this.jjMoveStringLiteralDfa1_0(0x200000000L);
		case 116:
			return this.jjMoveStringLiteralDfa1_0(0x2000L);
		case 119:
			return this.jjMoveStringLiteralDfa1_0(0x100000000L);
		case 123:
			return this.jjStopAtPos(0, 16);
		case 124:
			return this.jjMoveStringLiteralDfa1_0(0x200000L);
		case 125:
			return this.jjStopAtPos(0, 17);
		default:
			return this.jjMoveNfa_0(1, 0);
		}
	}

	private int jjMoveStringLiteralDfa1_0(long active0) {
		try {
			curChar = input_stream.readChar();
		} catch (java.io.IOException e) {
			this.jjStopStringLiteralDfa_0(0, active0);
			return 1;
		}
		switch (curChar) {
		case 38:
			if ((active0 & 0x100000L) != 0L)
				return this.jjStopAtPos(1, 20);
			break;
		case 61:
			if ((active0 & 0x1000000L) != 0L)
				return this.jjStopAtPos(1, 24);
			break;
		case 97:
			return this.jjMoveStringLiteralDfa2_0(active0, 0x4000L);
		case 101:
			return this.jjMoveStringLiteralDfa2_0(active0, 0x200000000L);
		case 102:
			if ((active0 & 0x80000000L) != 0L)
				return this.jjStartNfaWithStates_0(1, 31, 4);
			break;
		case 104:
			return this.jjMoveStringLiteralDfa2_0(active0, 0x100000000L);
		case 108:
			return this.jjMoveStringLiteralDfa2_0(active0, 0x40000000L);
		case 110:
			if ((active0 & 0x400000000L) != 0L)
				return this.jjStartNfaWithStates_0(1, 34, 4);
			break;
		case 114:
			return this.jjMoveStringLiteralDfa2_0(active0, 0x3000L);
		case 117:
			return this.jjMoveStringLiteralDfa2_0(active0, 0x800000000L);
		case 124:
			if ((active0 & 0x200000L) != 0L)
				return this.jjStopAtPos(1, 21);
			break;
		default:
			break;
		}
		return this.jjStartNfa_0(0, active0);
	}

	private int jjMoveStringLiteralDfa2_0(long old0, long active0) {
		if (((active0 &= old0)) == 0L)
			return this.jjStartNfa_0(0, old0);
		try {
			curChar = input_stream.readChar();
		} catch (java.io.IOException e) {
			this.jjStopStringLiteralDfa_0(1, active0);
			return 2;
		}
		switch (curChar) {
		case 105:
			return this.jjMoveStringLiteralDfa3_0(active0, 0x100000000L);
		case 108:
			return this.jjMoveStringLiteralDfa3_0(active0, 0x4000L);
		case 111:
			return this.jjMoveStringLiteralDfa3_0(active0, 0x1000L);
		case 115:
			return this.jjMoveStringLiteralDfa3_0(active0, 0x40000000L);
		case 116:
			if ((active0 & 0x800000000L) != 0L)
				return this.jjStartNfaWithStates_0(2, 35, 4);
			return this.jjMoveStringLiteralDfa3_0(active0, 0x200000000L);
		case 117:
			return this.jjMoveStringLiteralDfa3_0(active0, 0x2000L);
		default:
			break;
		}
		return this.jjStartNfa_0(1, active0);
	}

	private int jjMoveStringLiteralDfa3_0(long old0, long active0) {
		if (((active0 &= old0)) == 0L)
			return this.jjStartNfa_0(1, old0);
		try {
			curChar = input_stream.readChar();
		} catch (java.io.IOException e) {
			this.jjStopStringLiteralDfa_0(2, active0);
			return 3;
		}
		switch (curChar) {
		case 99:
			if ((active0 & 0x1000L) != 0L)
				return this.jjStartNfaWithStates_0(3, 12, 4);
			break;
		case 101:
			if ((active0 & 0x2000L) != 0L)
				return this.jjStartNfaWithStates_0(3, 13, 4);
			else if ((active0 & 0x40000000L) != 0L)
				return this.jjStartNfaWithStates_0(3, 30, 4);
			break;
		case 108:
			return this.jjMoveStringLiteralDfa4_0(active0, 0x100000000L);
		case 115:
			return this.jjMoveStringLiteralDfa4_0(active0, 0x4000L);
		case 117:
			return this.jjMoveStringLiteralDfa4_0(active0, 0x200000000L);
		default:
			break;
		}
		return this.jjStartNfa_0(2, active0);
	}

	private int jjMoveStringLiteralDfa4_0(long old0, long active0) {
		if (((active0 &= old0)) == 0L)
			return this.jjStartNfa_0(2, old0);
		try {
			curChar = input_stream.readChar();
		} catch (java.io.IOException e) {
			this.jjStopStringLiteralDfa_0(3, active0);
			return 4;
		}
		switch (curChar) {
		case 101:
			if ((active0 & 0x4000L) != 0L)
				return this.jjStartNfaWithStates_0(4, 14, 4);
			else if ((active0 & 0x100000000L) != 0L)
				return this.jjStartNfaWithStates_0(4, 32, 4);
			break;
		case 114:
			return this.jjMoveStringLiteralDfa5_0(active0, 0x200000000L);
		default:
			break;
		}
		return this.jjStartNfa_0(3, active0);
	}

	private int jjMoveStringLiteralDfa5_0(long old0, long active0) {
		if (((active0 &= old0)) == 0L)
			return this.jjStartNfa_0(3, old0);
		try {
			curChar = input_stream.readChar();
		} catch (java.io.IOException e) {
			this.jjStopStringLiteralDfa_0(4, active0);
			return 5;
		}
		switch (curChar) {
		case 110:
			if ((active0 & 0x200000000L) != 0L)
				return this.jjStartNfaWithStates_0(5, 33, 4);
			break;
		default:
			break;
		}
		return this.jjStartNfa_0(4, active0);
	}

	private int jjStartNfaWithStates_0(int pos, int kind, int state) {
		jjmatchedKind = kind;
		jjmatchedPos = pos;
		try {
			curChar = input_stream.readChar();
		} catch (java.io.IOException e) {
			return pos + 1;
		}
		return this.jjMoveNfa_0(state, pos + 1);
	}

	static final long[] jjbitVec0 = { 0x0L, 0x0L, 0xffffffffffffffffL, 0xffffffffffffffffL };

	private int jjMoveNfa_0(int startState, int curPos) {
		int startsAt = 0;
		jjnewStateCnt = 17;
		int i = 1;
		jjstateSet[0] = startState;
		int kind = 0x7fffffff;
		for (;;) {
			if (++jjround == 0x7fffffff)
				this.ReInitRounds();
			if (curChar < 64) {
				long l = 1L << curChar;
				do
					switch (jjstateSet[--i]) {
					case 1:
						if ((0x3ff000000000000L & l) != 0L) {
							if (kind > 8)
								kind = 8;
							this.jjCheckNAdd(0);
						} else if (curChar == 47)
							this.jjAddStates(0, 1);
						else if (curChar == 45)
							this.jjCheckNAdd(2);
						break;
					case 6:
						if (curChar == 42)
							this.jjCheckNAddTwoStates(12, 13);
						else if (curChar == 47)
							this.jjCheckNAddStates(2, 4);
						break;
					case 0:
						if ((0x3ff000000000000L & l) == 0L)
							break;
						if (kind > 8)
							kind = 8;
						this.jjCheckNAdd(0);
						break;
					case 2:
						if ((0x3ff000000000000L & l) == 0L)
							break;
						if (kind > 9)
							kind = 9;
						this.jjCheckNAdd(2);
						break;
					case 4:
						if ((0x3ff000000000000L & l) == 0L)
							break;
						if (kind > 36)
							kind = 36;
						jjstateSet[jjnewStateCnt++] = 4;
						break;
					case 5:
						if (curChar == 47)
							this.jjAddStates(0, 1);
						break;
					case 7:
						if ((0xffffffffffffdbffL & l) != 0L)
							this.jjCheckNAddStates(2, 4);
						break;
					case 8:
						if (((0x2400L & l) != 0L) && (kind > 6))
							kind = 6;
						break;
					case 9:
						if ((curChar == 10) && (kind > 6))
							kind = 6;
						break;
					case 10:
						if (curChar == 13)
							jjstateSet[jjnewStateCnt++] = 9;
						break;
					case 11:
						if (curChar == 42)
							this.jjCheckNAddTwoStates(12, 13);
						break;
					case 12:
						if ((0xfffffbffffffffffL & l) != 0L)
							this.jjCheckNAddTwoStates(12, 13);
						break;
					case 13:
						if (curChar == 42)
							this.jjCheckNAddStates(5, 7);
						break;
					case 14:
						if ((0xffff7bffffffffffL & l) != 0L)
							this.jjCheckNAddTwoStates(15, 13);
						break;
					case 15:
						if ((0xfffffbffffffffffL & l) != 0L)
							this.jjCheckNAddTwoStates(15, 13);
						break;
					case 16:
						if ((curChar == 47) && (kind > 7))
							kind = 7;
						break;
					default:
						break;
					}
				while (i != startsAt);
			} else if (curChar < 128) {
				long l = 1L << (curChar & 077);
				do
					switch (jjstateSet[--i]) {
					case 1:
						if ((0x7fffffe07fffffeL & l) == 0L)
							break;
						if (kind > 36)
							kind = 36;
						this.jjCheckNAdd(4);
						break;
					case 4:
						if ((0x7fffffe87fffffeL & l) == 0L)
							break;
						if (kind > 36)
							kind = 36;
						this.jjCheckNAdd(4);
						break;
					case 7:
						this.jjAddStates(2, 4);
						break;
					case 12:
						this.jjCheckNAddTwoStates(12, 13);
						break;
					case 14:
					case 15:
						this.jjCheckNAddTwoStates(15, 13);
						break;
					default:
						break;
					}
				while (i != startsAt);
			} else {
				int i2 = (curChar & 0xff) >> 6;
				long l2 = 1L << (curChar & 077);
				do
					switch (jjstateSet[--i]) {
					case 7:
						if ((jjbitVec0[i2] & l2) != 0L)
							this.jjAddStates(2, 4);
						break;
					case 12:
						if ((jjbitVec0[i2] & l2) != 0L)
							this.jjCheckNAddTwoStates(12, 13);
						break;
					case 14:
					case 15:
						if ((jjbitVec0[i2] & l2) != 0L)
							this.jjCheckNAddTwoStates(15, 13);
						break;
					default:
						break;
					}
				while (i != startsAt);
			}
			if (kind != 0x7fffffff) {
				jjmatchedKind = kind;
				jjmatchedPos = curPos;
				kind = 0x7fffffff;
			}
			++curPos;
			if ((i = jjnewStateCnt) == (startsAt = 17 - (jjnewStateCnt = startsAt)))
				return curPos;
			try {
				curChar = input_stream.readChar();
			} catch (java.io.IOException e) {
				return curPos;
			}
		}
	}

	static final int[] jjnextStates = { 6, 11, 7, 8, 10, 13, 14, 16, };

	/** Token literal values. */
	public static final String[] jjstrLiteralImages = { "", null, null, null, null, null, null, null, null, null, "\50",
			"\51", "\160\162\157\143", "\164\162\165\145", "\146\141\154\163\145", "\41", "\173", "\175", "\73", "\75",
			"\46\46", "\174\174", "\74", "\76", "\75\75", "\57", "\53", "\55", "\52", "\54", "\145\154\163\145",
			"\151\146", "\167\150\151\154\145", "\162\145\164\165\162\156", "\151\156", "\157\165\164", null, };

	/** Lexer state names. */
	public static final String[] lexStateNames = { "DEFAULT", };
	static final long[] jjtoToken = { 0x1fffffff01L, };
	static final long[] jjtoSkip = { 0xfeL, };
	protected SimpleCharStream input_stream;
	private final int[] jjrounds = new int[17];
	private final int[] jjstateSet = new int[34];
	protected char curChar;

	/** Constructor. */
	public WhileParserTokenManager(SimpleCharStream stream) {
		if (SimpleCharStream.staticFlag)
			throw new Error("ERROR: Cannot use a static CharStream class with a non-static lexical analyzer.");
		input_stream = stream;
	}

	/** Constructor. */
	public WhileParserTokenManager(SimpleCharStream stream, int lexState) {
		this(stream);
		this.SwitchTo(lexState);
	}

	/** Reinitialise parser. */
	public void ReInit(SimpleCharStream stream) {
		jjmatchedPos = jjnewStateCnt = 0;
		curLexState = defaultLexState;
		input_stream = stream;
		this.ReInitRounds();
	}

	private void ReInitRounds() {
		int i;
		jjround = 0x80000001;
		for (i = 17; i-- > 0;)
			jjrounds[i] = 0x80000000;
	}

	/** Reinitialise parser. */
	public void ReInit(SimpleCharStream stream, int lexState) {
		this.ReInit(stream);
		this.SwitchTo(lexState);
	}

	/** Switch to specified lex state. */
	public void SwitchTo(int lexState) {
		if ((lexState >= 1) || (lexState < 0))
			throw new TokenMgrError("Error: Ignoring invalid lexical state : " + lexState + ". State unchanged.",
					TokenMgrError.INVALID_LEXICAL_STATE);
		else
			curLexState = lexState;
	}

	protected Token jjFillToken() {
		final Token t;
		final String curTokenImage;
		final int beginLine;
		final int endLine;
		final int beginColumn;
		final int endColumn;
		String im = jjstrLiteralImages[jjmatchedKind];
		curTokenImage = (im == null) ? input_stream.GetImage() : im;
		beginLine = input_stream.getBeginLine();
		beginColumn = input_stream.getBeginColumn();
		endLine = input_stream.getEndLine();
		endColumn = input_stream.getEndColumn();
		t = Token.newToken(jjmatchedKind, curTokenImage);

		t.beginLine = beginLine;
		t.endLine = endLine;
		t.beginColumn = beginColumn;
		t.endColumn = endColumn;

		return t;
	}

	int curLexState = 0;
	int defaultLexState = 0;
	int jjnewStateCnt;
	int jjround;
	int jjmatchedPos;
	int jjmatchedKind;

	/** Get the next Token. */
	public Token getNextToken() {
		Token matchedToken;
		int curPos = 0;

		EOFLoop: for (;;) {
			try {
				curChar = input_stream.BeginToken();
			} catch (java.io.IOException e) {
				jjmatchedKind = 0;
				matchedToken = this.jjFillToken();
				return matchedToken;
			}

			try {
				input_stream.backup(0);
				while ((curChar <= 32) && ((0x100003600L & (1L << curChar)) != 0L))
					curChar = input_stream.BeginToken();
			} catch (java.io.IOException e1) {
				continue EOFLoop;
			}
			jjmatchedKind = 0x7fffffff;
			jjmatchedPos = 0;
			curPos = this.jjMoveStringLiteralDfa0_0();
			if (jjmatchedKind != 0x7fffffff) {
				if ((jjmatchedPos + 1) < curPos)
					input_stream.backup(curPos - jjmatchedPos - 1);
				if ((jjtoToken[jjmatchedKind >> 6] & (1L << (jjmatchedKind & 077))) != 0L) {
					matchedToken = this.jjFillToken();
					return matchedToken;
				} else
					continue EOFLoop;
			}
			int error_line = input_stream.getEndLine();
			int error_column = input_stream.getEndColumn();
			String error_after = null;
			boolean EOFSeen = false;
			try {
				input_stream.readChar();
				input_stream.backup(1);
			} catch (java.io.IOException e1) {
				EOFSeen = true;
				error_after = curPos <= 1 ? "" : input_stream.GetImage();
				if ((curChar == '\n') || (curChar == '\r')) {
					error_line++;
					error_column = 0;
				} else
					error_column++;
			}
			if (!EOFSeen) {
				input_stream.backup(1);
				error_after = curPos <= 1 ? "" : input_stream.GetImage();
			}
			throw new TokenMgrError(EOFSeen, curLexState, error_line, error_column, error_after, curChar,
					TokenMgrError.LEXICAL_ERROR);
		}
	}

	private void jjCheckNAdd(int state) {
		if (jjrounds[state] != jjround) {
			jjstateSet[jjnewStateCnt++] = state;
			jjrounds[state] = jjround;
		}
	}

	private void jjAddStates(int start, int end) {
		do
			jjstateSet[jjnewStateCnt++] = jjnextStates[start];
		while (start++ != end);
	}

	private void jjCheckNAddTwoStates(int state1, int state2) {
		this.jjCheckNAdd(state1);
		this.jjCheckNAdd(state2);
	}

	private void jjCheckNAddStates(int start, int end) {
		do
			this.jjCheckNAdd(jjnextStates[start]);
		while (start++ != end);
	}

}
